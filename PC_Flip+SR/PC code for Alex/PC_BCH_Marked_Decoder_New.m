...
function hat_coded_bits=PC_BCH_Marked_Decoder_New(coded_bits,BCHnu,BCHt,BCHe,It,llr,R,varargin)
...
for ll=1:It
        ...
        if CheckConflict  % detects miscorrection
           ...
           if ~Fail 
               ...
               if MiscDetFlag<2  % Flip bits after miscorrection detected
                ...
                if ~(status(rr)==0 || status(rr)==1)     % Decoding failure after miscorrection detected
                hat_coded_bits(rr,(Ir(rr,1:Nflip)))=~hat_coded_bits(rr,(Ir(rr,1:Nflip))); %Flips bits back after unresolved miscorrection detection
    ...
    for cc=1:BCHn+BCHe 
      ...
      if ll<=floor(ItThresh)                 % Miscorrection detection for first ItThresh-1 iterations        
        ...
        if CheckConflict  % Decode if no conflict
         MiscDetFlag= MiscDetFlag+1;
            ...
            if ~Fail 
             ...
             if ~(status(cc)==0 || status(cc)==1)     % Decoding failure after miscorrection detected
                    hat_coded_bits((Ic(1:Nflip,cc)),cc)=~hat_coded_bits((Ic(1:Nflip,cc)),cc); % Flips bits back after unresolved miscorrection detection
                    break %  exit miscorrection detection loop 
             end
             else 
