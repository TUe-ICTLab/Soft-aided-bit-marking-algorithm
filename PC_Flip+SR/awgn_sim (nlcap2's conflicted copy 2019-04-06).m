...
% Implements standard iBDD decoding using scaled
...
M=4;                   % Constellation cardinality/per dimension 
...
flag_demapper=0;       % Max-log algorithm for LLR calculation
L=de2bi(0:M-1,m);
...
semilogy(snrdB,BER,'bs-');
grid on;hold on;
axis([5,8,1e-9,1e-1]);

if Save 
    if isunix
s_path='/home/gliga/Dropbox/Work/TUe/MATLAB/MATLAB Sims Data/EnhancedHDdecoding/MarkedSR_PCdecoding/NewResults/SABM/';
    elseif ispc
...
switch DecoderType
    ...
    case 'SABM'
    filename=['iBDD-SABM_PAM' num2str(M) '_PC_BCH_n=' num2str(BCHn) '_t=' num2str(BCHt) '_e=' num2str(BCHe) '_NoIt=' num2str(I) '_R=' num2str(R) '_SABM_NoIt=' num2str(ItThresh) '_HighAccuracy.mat'] ;
    save([s_path filename],'snrdB','BERout_unc','SERout_unc','BERout','BERunc','SERunc','BER','r','R');
    case 'SABM-SR'
    filename=['iBDD-SABM-SR_PAM' num2str(M) '_PC_BCH_n=' num2str(BCHn) '_t=' num2str(BCHt) '_e=' num2str(BCHe) '_NoIt=' num2str(I) '_MarkingIter= ' num2str(ItThresh-.5) '_R='  num2str(R) '#3.mat'] ;
    save([s_path filename],'snrdB','BERout_unc','SERout_unc','BERout','BERunc','SERunc','BER','r','R','w');
    case 'SABM-SR-ST'
    filename=['iBDD-SABM-SR-ST_PAM' num2str(M) '_PC_BCH_n=' num2str(BCHn) '_t=' num2str(BCHt) '_e=' num2str(BCHe) '_NoIt=' num2str(I) '_MarkingIter= ' num2str(ItThresh-.5) '_R='  num2str(R) '.mat'] ;
    save([s_path filename],'snrdB','BERout_unc','SERout_unc','BERout','BERunc','SERunc','BER','r','R','w');
end
