 ...
 * Caution:
 *  Adjust the codeword s.t. it's compatible with the MATLAB built-in function bchenc.
...
static unsigned char *data = NULL; // ^ operation prohibits float and double type.
...
/*
 * Function: bchenc
 * Description: Perform BCH encoding.
...
void bchenc(int m_in, int length_in, int t_in, unsigned char *data_in)
{
    ...
    // Encode
    data = data_in;     // Pointer to data block
