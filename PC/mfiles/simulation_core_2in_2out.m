function [PosFECBER,GMIav,HDMIav]=simulation_core_2in_2out(m,EsN0dB,BCH,R,flag_display)
...
NBlocks=51;                        % Number of SCC blocks to transmit
...
FERC        = 0;
sentblocks  = 0;
pref_save_dir   = strcat('results/',R_str,'/',num2str(2^m),'PAM_2in_2out/');
...
while flag_end
    ...
    [hat_infbits,~]=StaircaseDecoding2in2out(hat_codedbits_preSCC,codedbits,BCH.m,BCH.t,R,BCH.w,BCH.iter);
    flag_display=1
    ...
    if sentblocks >= Maxblocks   
        ...
        pref_save_dir_in   = strcat('results/',R_str,'/',num2str(2^m),'PAM_2in_2out/incomplete/');
