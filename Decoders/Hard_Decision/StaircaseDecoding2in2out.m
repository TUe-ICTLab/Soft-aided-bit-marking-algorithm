...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [Decode_Information,Full_DecodeWord]=StaircaseDecoding2in2out(Received_Codeword,Transmitted_data,Staircasemm,Staircasett,StaircaseRR,Window_Length,Iteration_Times)
...
Num_of_decoding_window=(Num_of_Blocks+1-Window_Length)/2+1;

for i=1:Num_of_decoding_window
    fprintf('x');
    % select Window_Length Blocks as a decoding group
    for j=1:Window_Length 
        Blocks_in_Window{1,j}=ReceivedData_in_Blocks{1,(i-1)*2+j};
        Data_in_Window{1,j}=TransmitData_in_Blocks{1,(i-1)*2+j};
    end
